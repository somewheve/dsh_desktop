...
impl SubagentManager {
    ...
    pub fn spawn(&mut self, parent_session_id: &str, depth: u32, max_depth: u32) -> Result<String> {
        if depth > max_depth {
...
mod tests {
    ...
    fn spawn_and_depth() {
        ...
        let id = mgr.spawn("s-parent", 1, 3).unwrap();
        ...
        assert!(mgr.spawn("s-parent", 4, 3).is_err(), "深度超限应失败");
    }
}
